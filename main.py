...
# sidenote -> conventional to use capital words when declaring constants

# width and height of our window/surface
...
# how many pixels to move per press
VEL = 5
...
def draw_window(yellow, red, red_bullets, yellow_bullets, red_health, yellow_health):
    WIN.blit(SPACE, (0, 0))
    pygame.draw.rect(WIN, BLACK, MIDDLE_BORDER)
...
# Yellow Spasheship Movements


def yellow_movements_handler(keys_pressed, yellow):
...
# red Spasheship Movementss


def red_movements_handler(keys_pressed, red):
    if keys_pressed[pygame.K_LEFT] and red.x-VEL > MIDDLE_BORDER.x:  # LEFT
        red.x -= VEL
...
def main():
    ...
    while run:
        ...
        winner_text = ""
        if red_health <= 0:
            winner_text = "Yellow wins the game"
        if yellow_health <= 0:
            winner_text = "Red wins the game"

        if winner_text != "":
            draw_winner(winner_text)
            break

        # listens to keyboard strokes
        keys_pressed = pygame.key.get_pressed()

        # yellow  movement
        yellow_movements_handler(keys_pressed, yellow)

        # red movement
        red_movements_handler(keys_pressed, red)
